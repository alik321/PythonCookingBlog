...
from main.forms import RecipiForm, ImageForm
from main.pernissons import UserHasPermissionMixin
...
#     resipes = Recipe.objects.filter(category_id=slug)
...
class CategoryDetailView(DetailView):
    ...
    def get_context_date(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['resipes'] = Recipe.objects.filter(category_id=self.slug)
        return context
...
#         context = super().get_context_data(**kwargs)
#         images = self.get_object().get_image
#         context['images'] = self.get_object().images.exclude(id=images.id)
#         return context
...
def add_recipe(request):
    ...
    if request.method == 'POST':
        recipe_form = RecipiForm(request.POST)
...
def update_recipe(request, pk):
    ...
    if request.user == recipe.user:
        ImageFormSet = modelformset_factory(Image, form=ImageForm, max_num=3)
        recipe_form = RecipiForm(request.POST or None, instance=recipe)
        ...
        return HttpResponse('<h1>403 Forbiden</h1>')

# def delete_recipe(request, pk):
#     recipe = get_object_or_404(Recipe, pk=pk)
#     if request.method =='POST':
#         recipe.delete()
#         messages.add_message(request, messages.SUCCESS, 'Succsessfuly deleted')
